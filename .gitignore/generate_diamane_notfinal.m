function [finalfile] = generate_diamane_notfinal(lengths,widths,stackings,layers)
...
for h=1:length(lengths)
    for g=1:length(widths)
        for idx=1:length(stackings)
            ...
            %% edge case 1:
            if sum(strcmpi(stackings, "ab") || strcmpi(stackings, "aa")) ~= 1
            ...
            [file,len,wid,stackings] = generate_graphene(length_sheet,width_sheet,stackings,layers);
            ...
            for i = 1:length(y_bottom)
                for j = 1:length(y_top)
            ...
            finalfile = sprintf(finalfilename,stackings,layers,length_sheet,width_sheet);
            
            script = 'in_%s_%dx%d.lmp';
            fid2 = sprintf(script, stackings,length_sheet,width_sheet);
            fid2 = fopen(fid2, 'w');           
            ...
            outfile = sprintf(output,stackings,length_sheet,width_sheet);
            fid = fopen(outfile,'w');
