%%% Afnan Mostafa
%%% 02/26/2023
%%% ### code to generate mono- or bilayer Germanene (AA or AB) ### %%%
%%% This code generates aa- or ab- stacked Germanene. 
%%% The number of layers is defined by the variable 'layers' and
...
if buckled == 1
    if strcmp(stacking, 'ab')
        ...
        ge_ge_height = 3.34;               %% final len 2.68 = 3.34 - delta
    ...
    if strcmp(stacking, 'ab')
        ...
        ge_ge_height = 2.68;               %% final len 2.68
    elseif strcmp(stacking, 'aa')
...
%% sheet dimension
length_sheet = 10.2;        %% in nm with buffer of 0.2
...
if layers > 1
    ...
    filename = sprintf(filename1,layers,stacking,floor(length_sheet),floor(width_sheet),isBuckle);
else
    filename1 = 'germanene_%d_lay_%dnmx%dnm_%s.data';
    filename = sprintf(filename1,layers,floor(length_sheet),floor(width_sheet),isBuckle);
end
...
if write
    ...
    fprintf(fid,'%g %g zlo zhi\n\n',-2*floor(ge_ge_height*6*layers),2*floor(ge_ge_height*6*layers));
    fprintf(fid,'Masses\n\n');
    ...
    for i = 1:layers
        for j = 1:atom_layer
            if strcmp(stacking, 'ab')
                fprintf(fid,'%g %g %g %g %g\n',(j+(p-1)*atom_layer),i,(coords(j,1)+(ge_ge*abs(1-rem(i,2)))),coords(j,2),coords(j,3)+(ge_ge_height*i));
            else
                fprintf(fid,'%g %g %g %g %g\n',(j+(p-1)*atom_layer),i,coords(j,1),coords(j,2),coords(j,3)+(ge_ge_height*i));
            end
        end
        p=p+1;
    end
